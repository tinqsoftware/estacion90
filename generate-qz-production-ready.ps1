# Script para generar certificados QZ multi-dominio para desarrollo y producción
param(
...
try {
    ...
    # Crear archivo de configuración OpenSSL para múltiples dominios
    $configContent = @"
[req]
...
"@
    ...
    Write-Host "📝 Creando configuración multi-dominio..." -ForegroundColor Cyan
    ...
    # Generar certificado autofirmado con múltiples dominios
    ...
    Write-Host "   📜 Certificado público: $publicCertFile" -ForegroundColor White
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Clave privada válida y consistente" -ForegroundColor Green
    } else {
        Write-Host "❌ Problema con clave privada: $keyCheck" -ForegroundColor Red
    }
    
    # Verificar certificado y mostrar información
    Write-Host ""
    ...
    Write-Host "   📍 Producción: https://$ProdDomain" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "✨ El certificado funcionará tanto en desarrollo ($DevDomain) como en producción ($ProdDomain)" -ForegroundColor Green
    
} catch {
    Write-Host "❌ Error: $_" -ForegroundColor Red
    Write-Host "💡 Verifica que OpenSSL esté instalado y funcionando correctamente" -ForegroundColor Yellow
    exit 1
}
