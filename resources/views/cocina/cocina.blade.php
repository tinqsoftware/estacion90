...
function loadOrdersByDate(dateString) {
    ...
    .then(pedidos => {
        ...
        const emptySlots = Math.max(6 - pendientesCount, 0);
    ...
    .catch(error => {
        ...
        document.getElementById('proceso-container').innerHTML = '<div class="error">Error al cargar órdenes</div>';
        document.getElementById('pendientes-container').innerHTML = '<div class="error">Error al cargar órdenes</div>';
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleDrop(e) {
        ...
        if (draggedElement && this !== draggedFromContainer) {
            ...
            const newStatus = this.id === 'proceso-container' ? 'en_proceso' : 'pendiente';
    ...
    Object.keys(itemsByProduct).forEach(productName => {
        ...
        <div class="order-item">
            ...
            <div class="status-buttons">
                <div class="status-btn ${order.estado === 'en_proceso' ? 'status-blue' : ''}"></div>
                <div class="status-btn ${order.estado === 'pendiente' ? 'status-red' : ''}"></div>
            </div>
        </div>`;
    });
    ...
    statusButtons.forEach(button => {
        button.addEventListener('click', function() {
            if (this.classList.contains('status-blue')) {
                updateItemStatus(order.id, this.closest('.order-item'), 'en_proceso');
            } else if (this.classList.contains('status-red')) {
                updateItemStatus(order.id, this.closest('.order-item'), 'pendiente');
            }
        });
    });
...
@keyframes highlight {
    ...
    50% { box-shadow: 0 0 0 10px rgba(44, 90, 160, 0); }
